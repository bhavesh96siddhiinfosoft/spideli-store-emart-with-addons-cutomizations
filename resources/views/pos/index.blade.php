...
<script>
    ...
    async function loadProducts(page = 1) {       
        ...
        try {
            ...
            docs.forEach((doc) => {
                ...
                /* The wholesale tier on the card, so a bulk price is visible while
                 * browsing rather than only once the modal is open. Variants show
                 * the cheapest tier, matching how the card shows a price range. */
                let wholesaleHtmlContent = '';
                if (data.wholesaleEnabled === true && data.wholesaleMinQty) {
                    let wholesaleCandidates = [];
                    ...
                    if (data.item_attribute && data.item_attribute.variants && data.item_attribute.variants.length > 0) {
                        data.item_attribute.variants.forEach(function (v) {
                            ...
                            if (vw !== undefined && vw !== null && vw !== '') {
                                wholesaleCandidates.push(parseFloat(vw));
                            }
                        });
                    } else if (data.wholesalePrice !== undefined && data.wholesalePrice !== null && data.wholesalePrice !== '') {
                        wholesaleCandidates.push(parseFloat(data.wholesalePrice));
                    }
     ...
     *
     * Reads the hidden inputs the product card already carries, the same way the
     * rest of this screen does. variantWholesalePrice is the selected variant's
     * own price when it has one; it wins over the product's, exactly as the cart
     * resolves it. */
    function showModalWholesale(productId, variantWholesalePrice) {
        ...
        var amount = parseFloat(price).toFixed(decimal_degits);
        var formatted = currencyAtRight ? amount + '' + currentCurrency : currentCurrency + '' + amount;
        var label = "{{ trans('lang.wholesale_from_units') }}".replace(':count', minQty);

        $('#modal_wholesale_value').text(formatted + ' ' + label);
    ...
    $(document).on("click", '#modal-add-cart-btn', async function (event) {
        ...
        /* The quantity break travels with the line so the cart can reprice it
         * when the quantity changes, rather than only at the moment it is added.
         * A variant's own wholesale price wins over the product's. */
        let wholesaleEnabled = $('#wholesale_enabled_' + id).val() === '1';
    ...
    $(document).on('change', '#modal-attributes-container .attribute-radio', function () {
        ...
        /* The chosen variant's own wholesale price wins over the product's, the
    ...
    $(document).on('click', '#place_order_btn', async function () {
       ...
       if (!selectedUser) {
        ...
        let orderData = {          
            ...
            /* A POS order belongs to the region its store trades in, the same as
             * an order placed from the app. */
            'regionId': await resolveVendorRegionId(vendorDetails),
        };
