            ...
            <div class="col-12">
                <div class="d-flex top-title-section pb-4 justify-content-between">
                    <div class="d-flex top-title-left align-self-center">
                        ...
                        <h3 class="mb-0">{{trans('lang.placed_orders')}}</h3>
               ...
               <div class="card border">
                 <div class="card-header d-flex justify-content-between align-items-center border-0">
                   <div class="card-header-title">
                    <h3 class="text-dark-2 mb-2 h4">{{trans('lang.placed_orders')}}</h3>
                    <p class="mb-0 text-dark-2">{{trans('lang.placedorder_table_text')}}</p>
                   </div>              
                 </div>
                 <div class="card-body">
    ...
    <script type="text/javascript">
        ...
        document.addEventListener("DOMContentLoaded", async function() {
            if (authRole === 'employee') {               
                ...
                ref = database.collection('vendor_orders').orderBy('createdAt', 'desc').where('vendor.author',"==",authorId).where('status','==','Order Placed');
                refData = database.collection('vendor_orders').where('isPosOrder','==',false).where('vendor.author',"==",user_id).where('status','==','Order Placed')

            }else{
                ref = database.collection('vendor_orders').orderBy('createdAt', 'desc').where('vendor.author',"==",user_id).where('status','==','Order Placed');
                refData = database.collection('vendor_orders').where('isPosOrder','==',false).where('vendor.author',"==",user_id).where('status','==','Order Placed');
            }
