...
@section('scripts')
    <script type="text/javascript">
        ...
        async function buildHTML(val) {
            ...
            /* A wholesale tier is appended to the price cell rather than given a
             * column of its own, so the list still reads as one price per row. */
            if (val.wholesaleEnabled === true && val.wholesalePrice != '' && val.wholesalePrice != null) {
                var wholesale_val = currencyAtRight ?
                    parseFloat(val.wholesalePrice).toFixed(decimal_degits) + '' + activeCurrency :
