...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
             ...
             * variant table is rebuilt, which does not happen on every product,
             * so the editor was never created and the plain textarea showed. */
            initWholesaleEditor();
            ...
            ref.get().then(async function(snapshots) {
                 ...
                 * editor has already replaced it by now, so writing to the
                            ...
                            $.each(variantsSet, function(key, variant) {
                                ...
                                if (wholesaleEnabled) {
                                    ...
                                    if (parseFloat(variant_wholesale_price) >= parseFloat(variant_price)) {
                                        $(".error_top").show();
                                        $(".error_top").html("");
                                        $(".error_top").append("<p>{{ trans('lang.variant_wholesale_price_less_than_price_error') }}</p>");
                                        window.scrollTo(0, 0);
                                        isValid = true;
                                        return false;
                                    }
