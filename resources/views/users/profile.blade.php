...
@section('scripts')
    ...
    <script>
        ...
        database.collection('users').doc(vendorUserId).get().then(async function(userSnapshots) {
            ...
            if (selectedRegionId == '' && userData.regionId != undefined && userData.regionId != null && userData.regionId != '') {
            ...
            if(authRole === 'vendor'){
                ...
                ref.get().then(async function(snapshots) {
                    ...
                    if (vendor.hasOwnProperty('zoneId') && vendor.zoneId != '') {
                        selectedZoneId = vendor.zoneId;
                        applyZoneSelection();
                    }
                    if (vendor.hasOwnProperty('regionId') && vendor.regionId != '') {
                        selectedRegionId = vendor.regionId;
                        applyRegionSelection();
                    }
        ...
        /* Published regions only, sorted by name. Regions are a short list, so the
         * filter and sort happen here rather than in the query - combining
         * where('publish') with orderBy('name') would demand a composite index. */
        async function loadRegions() {
            ...
            regions.forEach(function(region) {
                ...
                $('#region_id').append($("<option></option>")
        ...
        /* Preselects the store's region once both the options and the store
         * record are in hand, whichever order they arrive in. */
        function applyRegionSelection() {
            ...
            if (selectedRegionId != '') {
                $('#region_id').val(selectedRegionId);
                renderZones();
            }

            /* The user profile shows the region as plain text - it is the store's
             * and is changed on the My Store screen, next to the zone it filters. */
            $('.user_region').text(regionNames[selectedRegionId] ? regionNames[selectedRegionId] : '-');
        }
        ...
        async function loadZones() {
            var snapshots = await database.collection('zone').where('publish', '==', true).orderBy('name', 'asc').get();

            zoneList = [];
            snapshots.docs.forEach(function(listval) {
                var data = listval.data();
                var area = [];
                data.area.forEach(function(location) {
                    area.push({
        ...
        function renderZones() {
            ...
            $('#zone').empty().append($("<option></option>")
                .attr("value", "")
            ...
            zoneList.forEach(function(zone) {
                ...
                $('#zone').append($("<option></option>")
                    .attr("value", zone.id)
                    .attr("data-area", JSON.stringify(zone.area))
                    .text(zone.name));
            });
        ...
        /* Restores the store's zone only when the chosen region still offers it;
         * changing region to one the old zone does not belong to clears it. */
        function applyZoneSelection() {
            if (selectedZoneId != '' && $('#zone option[value="' + selectedZoneId + '"]').length > 0) {
        ...
        $(".save_vendor_btn").click(async function(e) {
            ...
            <?php if(Route::is('store')): ?>
                ...
                jQuery("#data-table_processing").hide();
                $(".error_top").show();
                $(".error_top").html("<p>{{ trans('lang.select_region_error') }}</p>");
                window.scrollTo(0, 0);
                return false;
            } else if (zoneId == '') {
                jQuery("#data-table_processing").hide();
