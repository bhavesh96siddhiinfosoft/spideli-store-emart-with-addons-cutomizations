...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
                        ...
                        <div class="card-body">
                            ...
                            <form class="form-horizontal form-material" name="loginwithphon" id="login-with-phone-box"
                                ...
                                <div class="form-group" id="region_div">
                                    <label class="text-dark">{{ trans('lang.region') }}<span class="required-field"></span></label>
                                    <select id="region_id" class="form-control">
                                        <option value="">{{ trans('lang.select_region') }}</option>
                                    </select>
                                    <input type="hidden" id="hidden_region" />
                                </div>
        ...
        <script type="text/javascript">
            ...
            var ref_regions = database.collection('regions');
            ...
            /* Published regions only, sorted by name. Regions are a short list, so
             * the filter and sort happen here rather than in the query -
             * combining where('publish') with orderBy('name') would demand a
             * composite index. */
            async function loadRegions() {
                var snapshots = await ref_regions.get();
            ...
            function sendOTP() {
                ...
                if (firstName == '') {
                    ...
                    $(".error_top").show();
                    $(".error_top").html("");
                    $(".error_top").append("<p>{{ trans('lang.select_region_error') }}</p>");
                    window.scrollTo(0, 0);
                } else {
                ...
                if (code == "") {
                    ...
                    window.confirmationResult.confirm(document.getElementById("verificationcode").value)
                        .then(async function(result) {
                            ...
                            var regionId = $('#hidden_region').val();
